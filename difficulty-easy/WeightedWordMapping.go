...
func WeightedWordMapping(words []string, weights []int) (r string) {
	chars, m, m2, c := "abcdefghijklmnopqrstuvwxyz", map[int]string{}, map[string]int{}, 0
	for i := len(chars) - 1; i >= 0; i-- {
		m[c] = string(chars[i])
		c++
	}
	for i := 0; i < len(chars); i++ {
		m2[string(chars[i])] = weights[i]
	}
