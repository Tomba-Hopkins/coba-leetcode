...
func MaxTwinSumOfAnLinkedList(head *ListNode9) int {
	slow, fast := head, head
	for fast != nil && fast.Next != nil {
		slow = slow.Next
		fast = fast.Next.Next
	}
	var prev *ListNode9 = nil
	curr := slow
...
func buatinNodeSambilLetdown(a []int) *ListNode9 {
	if len(a) == 0 {
		return nil
	}
	head := &ListNode9{Val: a[0]}
	curr := head
	for i := 1; i < len(a); i++ {
		curr.Next = &ListNode9{Val: a[i]}
		curr = curr.Next
	}
	return head
}
